...
def get_axis(universe: mda.Universe, endpoints: list, type: str ="resid"):
    ...
    # Cost function to alter the weight of a ridge from Voronoi diagram
...
def get_atoms(universe: mda.Universe, axes: list, beta: float=4.0):
...
def get_projection(points: list, axes: list):
    """
...
def construct_tunnel(universe: mda.Universe, endpoints: list, type: str ="resid"):
    ...
    """
    This is a master function to realize a tunnel construction and return the atoms lining the tunnel, the axis found by the script and the atoms' cylindrical coordinates. For seperate functions, use tunnelwiz.builder.foo .

    :param universe:  biomolecule loaded with mda
    :param endpoints: list of: resids/atomids/[cartesian coordinates] which will be the descriptors of then tunnel exits
    :param type: how are the endpoints realized
    """

    axis = get_axis(universe, endpoints, type)
    tunnel_atoms = get_atoms(universe, axis)
    cyl_coor = get_projection(tunnel_atoms.positions, axis)

    return tunnel_atoms, axis, cyl_coor
