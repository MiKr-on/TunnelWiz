...
def show_heatmap(df: pd.DataFrame, type: str="Charge", xbin_size: int=3, ybin_size: int=1):
    ...
    :param df: dataframe created using "make_df" or being the same style
    ...
    y_bins = [-3.5+i*ybin_size for i in range(int(8/ybin_size))]
    y_label = [3.5-i*ybin_size for i in range(int(8/ybin_size))]
...
def show_scatter(df: pd.DataFrame, type: str="Chain"):
    """
    Create a scatter plot to visualize the inner surface of the tunnel. Suitable for showing different chains present in the complex molecule. Works with numerical descriptors as well as string ones - e.g. chain ID, type of atoms residue names and such.
     
    :param df: dataframe created using "make_df" or being the same style
    :param type: characteristic to be plotted out, must match the column names in df
    """

    fig = px.scatter(df, x="Z", y="Theta", color=type, size="Rinv", hover_data=["Resid",type],width=800,height=300,opacity=1)
    fig.update_layout(plot_bgcolor="white")
    fig.update_yaxes(
        range=[-3.7,3.7],
        title="Theta (rad)",
